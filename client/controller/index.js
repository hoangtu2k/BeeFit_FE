  ...
  return {
      ...
      findItemIndexById: function(itemId,idColor,idSize) {
        ...
        for (var i = 0; i < cartItems.length; i++) {
            ...
            if (cartItems[i].idProduct.id == itemId && cartItems[i].idColor == idColor && cartItems[i].idSize == idSize) {
...
app.run(function ($rootScope, $http, AuthService, CartService) {
  ...
  if (AuthService.getToken() != null) {
    ...
    $http({
      ...
      $http.get('http://localhost:8080/api/customer/getByUsername/' + username.data.username).then(function (user) {
        ...
        $http.get("http://localhost:8080/api/cart/getCartByCustomer/" + user.data.id).then(function (idd) {
          ...
          $rootScope.giamIndex = function (idCartDetail, IdProduct, idColor, idSize) {
          ...
          $rootScope.tangIndex = function (idCartDetail, IdProduct, idColor, idSize) {
    ...
    //giảm số lượng trong cart
    $rootScope.giamIndex1 = function (idProduct, idColor, idSize) {
      var getQuanity = parseInt(
        document.getElementById("qtyIndex1" + idProduct + "color" + idColor + "size" + idSize).value
      );
      ...
      //nếu product về số lượng là 0 thì check có thể xóa
      if (getQuanity <= 0) {
        $rootScope.deleteByCartIndex1(idProduct, idColor, idSize);
        getQuanity = 1;
        return;
      }
      //get đơn giá ở thời điểm hiện tại
    ...
    // tăng số lượng trong giỏ
    $rootScope.tangIndex1 = function (idProduct, idColor, idSize) {
      // check số lượng của sản phẩm đang còn
      ...
      $http({
        ...
        if (
          document.getElementById("qtyIndex1" + idProduct + "color" + idColor + "size" + idSize).value >= $rootScope.quantity
        ) {
          Swal.fire(
            "Số lượng đã đến mức tối đa số lượng sản phẩm hiện có !",
            "",
        ...
        document.getElementById("qtyIndex1" + idProduct + "color" + idColor + "size" + idSize).value =
          parseInt(document.getElementById("qtyIndex1" + idProduct + "color" + idColor + "size" + idSize).value) + 1;
    ...
    $rootScope.EnterQuantityIndex1 = function (
      ...
      var numberRegex = /^[0-9]+$/;
      if (
        !numberRegex.test(document.getElementById("qtyIndex1" + idProduct + "color" + idColor + "size" + idSize).value)
      ) {
        Swal.fire("Số lượng phải là số nguyên dương !!", "", "error");
        document.getElementById("qtyIndex1" + idProduct + "color" + idColor + "size" + idSize).value = 1;
        return;
      }
      var params = {
        IdProduct: idProduct,
          ...
          $http
            ...
            .then(function (response) {
              ...
              var cartUpdate = {
                ...
                idColor: idColor,
                idSize: idSize,
                quantity: parseInt(document.getElementById("qtyIndex1" + idProduct + "color" + idColor + "size" + idSize).value),
                unitPrice: unitPrice
              }
              CartService.updateCartItem(index, cartUpdate);
              // Recalculate total price
